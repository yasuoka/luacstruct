local main = function()
    ...
    local test_extra = require("test_extra")
    --
    -- reference to another instance
    --
    local ref = test_extra.test_ref()
    assert(ref)
    assert(ref.a == 1)
    assert(ref.b == 2)
    assert(ref.ref)
    ...
    -- nested structure
    --
    local nest = test_extra.test_nest()
    assert(nest)
    assert(nest.a == 1)
    assert(nest.b == 2)
    assert(nest.nest)
    ...
    local color,m = test_extra.test_enum()
    ...
    f1,f2,m1,m2 = test_extra.test_copy()
    ...
    local m1,m2,int8a,int8b = test_extra.test_array()
    -- check __len
    assert(#m1.int4 == 4)
    assert(#m1.sub2 == 2)
    assert(#m1.sub3 == 3)
